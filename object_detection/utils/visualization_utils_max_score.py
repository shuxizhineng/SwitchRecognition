...
def visualize_boxes_and_labels_on_image_array(image,
  ...
  #get the index of the highest score of classes , save to idxDict
  idxDict = {}
  if scores is not None:                
    tmp_classes = classes[:min(max_boxes_to_draw, boxes.shape[0])]
    for k, v in enumerate(tmp_classes):                           
      if idxDict.has_key(str(v)):                                 
        continue                                                  
      else:                                                       
        idxDict[str(v)] = k                                       
      if len(idxDict) == 2:                                       
